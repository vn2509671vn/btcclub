<?php 
    ...
    function danhsach(){
        $query = "SELECT * FROM nguoidung where nguoidung_taikhoan not like 'admin'";   
        return mysql_query($query);
    }
    ...
    function cayf1($source, $parent, &$newString){
    	if(count($source) > 0) {
    		$newString .= '<ul>';
    		foreach ($source as $key => $value){
    		    if( mysql_num_rows(getparent($source[$key]['nguoidung_id'])) != 3){
        			if($value['nguoidung_parent_id'] == $parent){
        			 //   $value['nguoidung_taikhoan'] = $value['nguoidung_taikhoan'] . '( ' . $value['nguoidung_loainhanh'] . ': ' . $value['nguoidung_giatricanbang'] . ' )' ;
        				$newString .= '<li class="closed">'. $value['nguoidung_taikhoan'] . '( ' . $value['nguoidung_loainhanh'] . ': ' . $value['nguoidung_giatricanbang'] . ' )' ;
        				unset($source[$key]);
        				$newParent = $value['nguoidung_id'];
        				cayf1($source, $newParent, $newString);
        				$newString .= '</li>';
        			}
    		    }
    		}
